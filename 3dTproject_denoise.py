...
def get_parser():
    parser = argparse.ArgumentParser(description='Denoise datasets using 3dTproject.')
...
def main(argv=None):
    ...
    if not op.isfile(op.join(args.work_dir, censoring_script)):
        shutil.copyfile(op.join(op.dirname(args.bids_dir), 'code', censoring_script), op.join(args.work_dir, censoring_script))
